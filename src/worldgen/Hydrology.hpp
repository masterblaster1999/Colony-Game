...
struct HeightField {
    ...
    [[nodiscard]] float& at(int x, int y) {
        ...
        return data[static_cast<size_t>(y) * static_cast<size_t>(w) + static_cast<size_t>(x)];
    }
...
struct HydroParams {
    ...
    float incisionK      = 0.015f;  // K
    float streamPowerM   = 0.5f;    // m
    float slopeExponentN = 1.0f;    // n
};
