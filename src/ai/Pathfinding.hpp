...
// Global Point because existing code used std::vector<Point> unqualified.
struct Point {
    int x{0};
    int y{0};
};
...
[[nodiscard]] PFResult aStar(const GridView& g,
                             Point start,
                             Point goal,
...
[[nodiscard]] inline std::vector<Point> aStar(const GridView& g,
                                              Point start,
                                              Point goal,
                                              int maxExpandedNodes = -1)
{
