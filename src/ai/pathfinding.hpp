...
private:
    ...
    static inline float octile(int x0, int y0, int x1, int y1, float straight, float diagonal) noexcept
    {
        ...
        const int dmax = dx ^ dy ? (dx + dy - dmin) : (dx + dy - dmin); // keep branchless; dmax = max(dx,dy)
        const int dmaxCalc = (dx > dy) ? dx : dy;
        // Using explicit dmax avoids subtlety above:
        return diagonal * float(dmin) + straight * float(dmaxCalc - dmin);
    }
