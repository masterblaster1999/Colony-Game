...
void DeviceResources::Initialize(HWND hwnd, UINT width, UINT height, bool vsync)
{
    ...
    EnableD3D12DebugLayer(gpuValidate);
#endif
...
void DeviceResources::CreateFactory()
{
    ...
    // (Present() takes this into account.)
}
...
void DeviceResources::CreateSwapChain()
{
    ...
    ThrowIfFailed(
        m_factory->MakeWindowAssociation(
            m_hwnd, DXGI_MWA_NO_ALT_ENTER | DXGI_MWA_NO_WINDOW_CHANGES),
    ...
    sc.Flags       = m_allowTearing
                   ? DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING
                   : 0; // allow tearing if supported
    ...
    // Flip-model & tearing follow Microsoft's modern DXGI guidance.
}
...
void DeviceResources::OnResize(UINT width, UINT height)
{
    ...
    if (width == m_width && height == m_height) return;  // no-op

    m_width  = width;
    m_height = height;

    // Ensure GPU is not using the old buffers
    WaitForGPU();

    // RELEASE ALL refs to back buffers, per DXGI docs
    ReleaseSwapChainResources();

    UINT flags = m_allowTearing ? DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING : 0;

    HRESULT hr = m_swapChain->ResizeBuffers(
        FrameCount, m_width, m_height, kBackBufferFormat, flags);

    if (FAILED(hr))
    {
        // If we still hold a reference to a back buffer, ResizeBuffers will fail.
        // Microsoft docs explicitly require releasing all direct/indirect refs first.
...
void DeviceResources::Present()
{
    ...
    // Tearing is only valid with sync interval 0.
...
{
    // Schedule a signal in the queue for the current frame.
...
void DeviceResources::HandleDeviceLost()
{
    ...
    if (m_device)
    {
        // Retrieve why the device was removed (for logging)
        reason = m_device->GetDeviceRemovedReason();
        (void)reason; // TODO: log this somewhere central
    }
