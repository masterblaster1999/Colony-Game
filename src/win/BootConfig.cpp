...
namespace
{
    ...
    {
        size_t i = 0, j = s.size();
        while (i < j && iswspace(static_cast<unsigned>(s[i]))) ++i;
        while (j > i && iswspace(static_cast<unsigned>(s[j - 1]))) --j;
    ...
    static bool TryParseMillisecondsW(const std::wstring& s, double& out) noexcept
    {
        ...
        if (t.size() >= 2) {
            wchar_t c1 = t[t.size() - 2], c2 = t[t.size() - 1];
        ...
        try {
            ...
            while (idx < t.size() && iswspace(static_cast<unsigned>(t[idx]))) ++idx;
...
BootConfig GetBootConfigFromEnv()
{
    ...
    if (auto mode = ReadEnvW(L"COLONY_PRESENT_MODE"); !mode.empty()) {
        ...
        // If you need a UTF-8 narrow copy for logging:
        // const std::string modeUtf8 = WideToUtf8(cfg.presentMode);
        // log("PresentMode=%s", modeUtf8.c_str());
    }
