...
namespace {
    ...
    void set_dpi_awareness()
    {
        ...
        if (user32) {
            ...
            if (pSet) { pSet(DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2); FreeLibrary(user32); return; }
            FreeLibrary(user32);
        }
...
void Preflight(const Options& opt)
{
    ...
    if (!dir_has_assets(g_root, opt.assetDirName)) {
        ...
        log_info(std::string("Assets folder present: ") + path_u8(g_root / opt.assetDirName));
    }
}
