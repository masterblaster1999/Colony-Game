...
uint32_t ReplayWriter::crc32(const uint8_t* p, size_t n){
    uint32_t c=~0u;
    for(size_t i=0;i<n;i++) c = kCrcTable[(c ^ p[i]) & 0xFFu] ^ (c >> 8);
    return ~c;
}
uint32_t ReplayReader::crc32(const uint8_t* p, size_t n){ return ReplayWriter::crc32(p,n); }

bool ReplayWriter::save(const std::string& path) const {
    std::ofstream f(path, std::ios::binary);
    if(!f) return false;
    f.write(reinterpret_cast<const char*>(&m_hdr), sizeof(m_hdr));
    if(!m_events.empty())
        f.write(reinterpret_cast<const char*>(m_events.data()), sizeof(InputEvent)*m_events.size());
    ...
    if(!m_events.empty())
        std::memcpy(tmp.data()+sizeof(m_hdr), m_events.data(), sizeof(InputEvent)*m_events.size());
...
bool ReplayReader::load(const std::string& path) {
    ...
    if(!f) return false;
    f.read(reinterpret_cast<char*>(&m_hdr), sizeof(m_hdr));
    ...
    uint32_t crcCalc = crc32(data.data(), size-sizeof(uint32_t));
    if(crcStored != crcCalc) return false;

    // events
    size_t eventBytes = size - sizeof(ReplayHeader) - sizeof(uint32_t);
    size_t count = eventBytes / sizeof(InputEvent);
    m_events.resize(count);
    if(count) std::memcpy(m_events.data(), data.data()+sizeof(ReplayHeader), count*sizeof(InputEvent));
    return true;
}
