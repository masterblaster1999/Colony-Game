...
#else
  // If this file ever lands on non-Windows CI, don't make it fail.
...
namespace cg_test
{
    ...
    static inline double height_at(int x, int y, std::uint32_t seed);
    ...
    // Try a project-local noise header (adjust the namespace/function if needed)
        ...
        static inline double height_at(int, int, std::uint32_t) {
    ...
    static inline std::vector<float> make_grid(std::uint32_t seed, int cx, int cy, int size) {
        ...
        for (int y = 0; y < size; ++y) {
            for (int x = 0; x < size; ++x) {
                ...
                g[std::size_t(y) * size + std::size_t(x)] =
                    static_cast<float>(height_at(wx, wy, seed));
            }
        }
        return g;
    }
...
{
    // MSVC C2512 fix: construct Approx with a value, then set epsilon.
    ...
    for (auto seed : seeds) {
        for (auto [x,y] : coords) {
            ...
            CHECK(b == doctest::Approx(a).epsilon(eps)); // <-- correct Approx usage
        }
    }
}
...
{
    // Use multiple seeds and several chunk coordinates including negatives.
    const std::array<std::uint32_t, 4> seeds{{
    ...
    constexpr int SIZE = 64; // adjust to your internal chunk size if you want
...
{
    // This is a sanity check to catch accidental seed-ignoring paths.
    ...
    CHECK(hA != hB); // non-deterministic worldgen would often violate this
}

TEST_CASE("Traversal order invariance: randomized sampling vs sequential")
{
    // Some pipelines accidentally depend on iteration order (e.g., hidden caches).
    // We probe that evaluating points in random order yields the same final grid data.
    ...
    // Hash both vectors – bitwise equality is even stronger than Approx.
...
// If we couldn't bind to any noise/worldgen header, keep CI green but tell you why.
