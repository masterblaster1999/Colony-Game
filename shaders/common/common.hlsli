...
// Amount of ordered-dither (in 0..1 LDR units), set 0 to disable
...
float3 cg_linear_to_srgb(float3 rgb) { return float3(cg_linear_to_srgb1(rgb.r), cg_linear_to_srgb1(rgb.g), cg_linear_to_srgb1(rgb.b)); }
float3 cg_srgb_to_linear(float3 rgb) { return float3(cg_srgb_to_linear1(rgb.r), cg_srgb_to_linear1(rgb.g), cg_srgb_to_linear1(rgb.b)); }
...
float3 cg_tonemap_hable(float3 x)
{
    ...
    float white = ((W * (A * W + C * B)) + D * E) / max((W * (A * W + B) + D * F), CG_EPS) - (E / F);
    return curve / white;
}
...
float cg_fog_height(float3 cameraPos, float3 worldPos, float density0, float falloff)
{
    ...
    float integral = (abs(ay) < 1e-4) ? (t0 * L) : (t0 * (1.0 - exp(-ay * L)) / max(ay, CG_EPS));
...
float4 cg_atlas_sample(Texture2D texAtlas, SamplerState samp, float2 tileUV, CGAtlasInfo info)
{
    ...
    uint w, h, mips; texAtlas.GetDimensions(0, w, h, mips);
