#pragma once
// JPS.hpp — Jump Point Search neighbor expander for uniform grids (header-only, MSVC/Windows friendly).
// Keeps your A* API: you still pop from OPEN, compute heuristic, etc. Only neighbor expansion changes.
//
// Grid requirements:
//   bool passable(int x, int y) const;   // false for OOB or blocked
//   bool allowDiagonal() const;          // true if 8-connected motion is allowed
//
// MIT-like: ship it as-is.
//
// References: Harabor & Grastien (AAAI'11, ICAPS'14). See README in PR.
...
struct Options {
    bool allowDiagonal = true;  // 8-connected grid
...
// Template so you can adapt any grid type that implements passable() and allowDiagonal().
template <class Grid>
class JPS {
public:
...
private:
    ...
    inline bool canStep(int x, int y, int dx, int dy) const {
        ...
        if (dx != 0 && dy != 0) {
            if (!m_opt.allowDiagonal || !m_grid.allowDiagonal()) return false;
    ...
    // Forced neighbor test (Harabor & Grastien 2011)
    ...
    // JPS "jump": march along (dx,dy) until goal, a forced neighbor, or blocked.
    bool jump(int x, int y, int dx, int dy, const Coord& goal, Coord& out, float& stepCost) const {
        ...
        for (;;) {
            ...
            if (diag) {
                // Diagonal: check hor/vert components for a jump point
            ...
            if (!canStep(cx, cy, dx, dy)) break; // blocked ahead → no jump point
        }
        return false;
    }

    // Pruned directions (natural + forced)
                          ...
                          detail::Dir* out, int& count) const
    {
        ...
        if (!parent) {
            for (auto d : detail::DIRS) {
                ...
                if (!(m_opt.allowDiagonal && m_grid.allowDiagonal()) && (d.dx != 0 && d.dy != 0)) continue;
        ...
        if (dx != 0 && dy != 0) { // diagonal natural: (dx,dy), (dx,0), (0,dy)
        ...
        if (dx != 0 && dy == 0) { // horizontal
            if (canStep(cur.x, cur.y, dx, 0)) out[count++] = {dx, 0};
            if (m_opt.allowDiagonal && m_grid.allowDiagonal()) {
