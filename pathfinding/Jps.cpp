...
struct Node {
    ...
    int   px = 0, py = 0;  // parent coordinates (for direction reconstruction)
    bool  opened = false;
    bool  closed = false;
};
...
inline bool can_step(const IGrid& g, int x, int y, int dx, int dy, const JpsOptions& o) {
    ...
    // Forbid cutting across corners if requested
...
// Slight tie-breaker to prefer straighter paths (keeps w=1 optimal)
...
inline bool has_forced_neighbors_diag(const IGrid& g, int x, int y, int dx, int dy) {
    // diagonal: two classical forced-neighbor patterns
                        ...
                        const JpsOptions& o, std::vector<std::pair<int,int>>& out)
{
    ...
    // No parent => return all legal base directions
    if (px == x && py == y) {
...
// ---------- Optional LOS smoothing (supercover) -------------------------------
