...
// Octile distance (Manhattan if diagonals disabled)
...
// Small tie-breaker to prefer straighter paths (optimal with weight=1)
...
// Forced-neighbor tests (straight)
...
// Forced-neighbor tests (diagonal)
...
// Enumerate pruned directions from parent direction
...
// Supercover LOS (optional smoothing)
...
std::vector<Cell> jps_find_path(const IGrid& grid, Cell start, Cell goal, const JpsOptions& opt)
{
    ...
    (void)H; // reserved for future guards
