...
[[nodiscard]] std::uint8_t SanitizePacked(int v) noexcept
{
    ...
    // Blueprints represent *plans* (and built tiles that can be expressed as plans).
    // Allow up through Door, but explicitly reject natural obstacles like Trees.
    const std::uint8_t maxTile = static_cast<std::uint8_t>(colony::proto::TileType::Bed);

    std::uint8_t safeTile = (tile <= maxTile)
        ? tile
        : static_cast<std::uint8_t>(colony::proto::TileType::Empty);

    if (safeTile == static_cast<std::uint8_t>(colony::proto::TileType::Tree))
        safeTile = static_cast<std::uint8_t>(colony::proto::TileType::Empty);
...
colony::proto::TileType BlueprintUnpackTile(std::uint8_t p) noexcept
{
    ...
    const auto t = static_cast<colony::proto::TileType>(tile);
    if (t == colony::proto::TileType::Tree)
        return colony::proto::TileType::Empty;
    return t;
}
