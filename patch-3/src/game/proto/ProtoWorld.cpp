...
[[nodiscard]] float TileNavCost(TileType t) noexcept
{
    ...
    case TileType::Bed:       return 1.10f; // furniture
...
float TileBuildTimeSeconds(TileType t) noexcept
{
    ...
    case TileType::Bed: return 1.10f;
...
void World::reset(int w, int h, std::uint32_t seed)
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        // Start everyone with a full personal rest reserve.
        const float maxPersonalRest = static_cast<float>(std::max(0.0, colonistMaxPersonalRest));
        c.personalRest = maxPersonalRest;
    ...
    // Build bed cache (for sleep jobs).
    rebuildBedCache();
...
double World::timeOfDay01() const noexcept
{
    const double dayLen = std::max(1e-6, dayLengthSeconds);
    double t = std::fmod(m_timeSeconds, dayLen);
    if (t < 0.0)
        t += dayLen;
    return t / dayLen;
}

bool World::isNight() const noexcept
{
    // Clamp to [0,1] to keep this robust if tuning is edited live.
    const double ns = std::clamp(nightStart01, 0.0, 1.0);
    const double ne = std::clamp(nightEnd01, 0.0, 1.0);
    if (ns == ne)
        return false;

    const double tod = timeOfDay01();

    // If start < end, night is a simple interval [start, end).
...
void World::tryStartQueuedManualOrders(Colonist& c) noexcept
{
    ...
    // Tired colonists should sleep before following manual orders.
    const float sleepThreshold = static_cast<float>(std::max(0.0,
        isNight() ? colonistSleepThresholdRestNight : colonistSleepThresholdRestDay));
    if (sleepThreshold > 0.0f && c.personalRest <= sleepThreshold)
        return;
...
bool World::DebugSetBuiltTile(int x, int y, TileType built, bool builtFromPlan) noexcept
{
    ...
    if (oldBuilt == TileType::Bed && built != TileType::Bed)
        bedCacheRemove(x, y);
    if (oldBuilt != TileType::Bed && built == TileType::Bed)
        bedCacheAdd(x, y);
    ...
    if (newBuilt == TileType::Bed)
        bedCacheAdd(targetX, targetY);
...
void World::tick(double dtSeconds)
{
    ...
    // Advance world clock (used for day/night behavior).
    ...
    // -----------------------------------------------------------------
    // Needs: personal rest (sleep)
    // -----------------------------------------------------------------
    const float restRate = static_cast<float>(std::max(0.0, restPerColonistPerSecond));
    const float maxRest = static_cast<float>(std::max(0.0, colonistMaxPersonalRest));
    const float sleepThreshold = static_cast<float>(std::max(
        0.0, isNight() ? colonistSleepThresholdRestNight : colonistSleepThresholdRestDay));

    for (Colonist& c : m_colonists)
    {
        c.personalFood = std::max(0.0f, c.personalFood - eatRate * dtF);

        // Clamp to current max (tuning can be edited live).
        c.personalRest = clampf(c.personalRest, 0.0f, maxRest);

        // Drain rest while awake.
        if (c.jobKind != Colonist::JobKind::Sleep)
            c.personalRest = std::max(0.0f, c.personalRest - restRate * dtF);
    }
    ...
    if (eatThreshold > 0.0f)
    {
        for (Colonist& c : m_colonists)
        {
            if (c.personalFood > eatThreshold || !c.hasJob)
                continue;
            if (c.jobKind == Colonist::JobKind::BuildPlan || c.jobKind == Colonist::JobKind::ManualMove ||
                c.jobKind == Colonist::JobKind::HaulWood || c.jobKind == Colonist::JobKind::Sleep)
                cancelJob(c);
        }
    }

    // If a colonist is getting tired, preempt non-essential work so they can
    // go sleep (hunger still takes priority over sleep).
    if (sleepThreshold > 0.0f)
    {
        for (Colonist& c : m_colonists)
        {
            if (c.personalRest > sleepThreshold || !c.hasJob)
                continue;
            if (c.jobKind == Colonist::JobKind::Eat || c.jobKind == Colonist::JobKind::Sleep)
                continue;

            if (c.jobKind == Colonist::JobKind::BuildPlan || c.jobKind == Colonist::JobKind::ManualMove ||
                c.jobKind == Colonist::JobKind::HaulWood || c.jobKind == Colonist::JobKind::Harvest)
            {
                cancelJob(c);
            }
        }
    ...
    // -----------------------------------------------------------------
    // Job assignment
    // Priority: Eat (hunger) -> Sleep (rest) -> manual queue -> Harvest -> Build -> Haul
    // -----------------------------------------------------------------
    assignEatJobs(dtSeconds);
    assignSleepJobs(dtSeconds);
    ...
    for (Colonist& c : m_colonists)
    {
        ...
        stepSleepingIfReady(c, dtSeconds);
    }
...
// -----------------------------------------------------------------------------
// Bed distance field (sleep)
// -----------------------------------------------------------------------------

std::uint32_t World::buildBedField() const
{
    ++m_pathStats.sleepFieldComputed;
    ...
    // Ensure scratch arrays are correctly sized.
    if (m_bedFieldDist.size() != n)
        m_bedFieldDist.assign(n, std::numeric_limits<float>::infinity());
    if (m_bedFieldParent.size() != n)
        m_bedFieldParent.assign(n, colony::pf::kInvalidNodeId);
    if (m_bedFieldStamp.size() != n)
        m_bedFieldStamp.assign(n, 0u);
    if (m_bedFieldBedKey.size() != n)
        m_bedFieldBedKey.assign(n, 0u);

    // Bump the stamp (with wraparound handling).
    ++m_bedFieldStampValue;
    if (m_bedFieldStampValue == 0u)
    {
        std::fill(m_bedFieldStamp.begin(), m_bedFieldStamp.end(), 0u);
        m_bedFieldStampValue = 1u;
    }
    const std::uint32_t stamp = m_bedFieldStampValue;

    auto getDist = [&](colony::pf::NodeId id) -> float {
        const std::size_t idx = static_cast<std::size_t>(id);
        return (m_bedFieldStamp[idx] == stamp) ? m_bedFieldDist[idx] : std::numeric_limits<float>::infinity();
    };

    auto getKey = [&](colony::pf::NodeId id) -> std::uint64_t {
        const std::size_t idx = static_cast<std::size_t>(id);
        return (m_bedFieldStamp[idx] == stamp) ? m_bedFieldBedKey[idx] : std::numeric_limits<std::uint64_t>::max();
    };

    auto setNode = [&](colony::pf::NodeId id, float d, colony::pf::NodeId parent, std::uint64_t bedKey) {
        const std::size_t idx = static_cast<std::size_t>(id);
        m_bedFieldStamp[idx] = stamp;
        m_bedFieldDist[idx] = d;
        m_bedFieldParent[idx] = parent;
        m_bedFieldBedKey[idx] = bedKey;
    };

    struct QNode
    {
        float d = 0.0f;
        colony::pf::NodeId id = colony::pf::kInvalidNodeId;
        bool operator>(const QNode& o) const noexcept { return d > o.d; }
    };

    std::priority_queue<QNode, std::vector<QNode>, std::greater<>> open;

    std::uint64_t sources = 0;

    // Seed all unreserved beds as distance-0 sources.
    for (const colony::pf::IVec2& p : m_bedCells)
    {
        if (!inBounds(p.x, p.y))
            continue;

        const Cell& c = cell(p.x, p.y);
        if (c.built != TileType::Bed)
            continue;
        if (c.bedReservedBy != -1)
            continue;
        if (!m_nav.passable(p.x, p.y))
            continue;

        const colony::pf::NodeId id = colony::pf::to_id(p.x, p.y, w);
        const std::uint64_t bedKey = PackPlanKey(p.x, p.y);

        const float oldD = getDist(id);
        const std::uint64_t oldK = getKey(id);

        // Prefer any newly discovered source, or a tie-breaking smaller bed key.
        if (0.0f < oldD || (oldD == 0.0f && bedKey < oldK))
        {
            setNode(id, 0.0f, colony::pf::kInvalidNodeId, bedKey);
            open.push({0.0f, id});
            ++sources;
        }
    }

    m_pathStats.sleepFieldSources += sources;
    ...
    while (!open.empty())
    {
        ...
        const colony::pf::IVec2 cpos = colony::pf::from_id(cur.id, w);
        const std::uint64_t curKey = m_bedFieldBedKey[static_cast<std::size_t>(cur.id)];
        ...
        for (int dy = -1; dy <= 1; ++dy)
        {
            for (int dx = -1; dx <= 1; ++dx)
            {
                ...
                const int nx = cpos.x + dx;
                const int ny = cpos.y + dy;
                if (!inBounds(nx, ny))
                    continue;
                ...
                // Diagonals require both cardinal steps to be passable.
                if (dx != 0 && dy != 0)
                {
                    if (!m_nav.passable(cpos.x + dx, cpos.y) || !m_nav.passable(cpos.x, cpos.y + dy))
                        continue;
                }

                const colony::pf::NodeId nid = colony::pf::to_id(nx, ny, w);
                const float stepCost = m_nav.step_cost(cpos.x, cpos.y, nx, ny);
                const float nd = cur.d + stepCost;

                const float oldD = getDist(nid);
                const std::uint64_t oldKey = getKey(nid);
                ...
                else if (nd == oldD && curKey < oldKey)
                {
                    // Tie-break: prefer the lexicographically smaller bed.
                    setNode(nid, nd, cur.id, curKey);
                    open.push({nd, nid});
                }
            }
...
bool World::queryBedField(std::uint32_t stamp, int startX, int startY,
                          ...
                          std::vector<colony::pf::IVec2>& outPath) const
{
    ...
    const int w = m_w;
    const std::size_t idx = static_cast<std::size_t>(colony::pf::to_id(startX, startY, w));
    if (idx >= m_bedFieldStamp.size() || m_bedFieldStamp[idx] != stamp)
        return false;

    const std::uint64_t bedKey = m_bedFieldBedKey[idx];
    outBedX = UnpackPlanX(bedKey);
    outBedY = UnpackPlanY(bedKey);

    // Reconstruct path by following parents towards the source bed.
    colony::pf::NodeId cur = static_cast<colony::pf::NodeId>(idx);
    while (cur != colony::pf::kInvalidNodeId)
    {
        const colony::pf::IVec2 p = colony::pf::from_id(cur, w);
        outPath.push_back(p);
        const std::size_t cidx = static_cast<std::size_t>(cur);
        cur = m_bedFieldParent[cidx];
    }

    // Safety: ensure the path starts at the requested start.
    if (outPath.empty() || outPath.front().x != startX || outPath.front().y != startY)
        return false;
...
bool World::findPathToNearestAvailableBed(int startX, int startY,
                                         int& outBedX, int& outBedY,
                                         std::vector<colony::pf::IVec2>& outPath) const
{
    outBedX = -1;
    ...
    // Quick reject: no beds at all.
    if (builtCount(TileType::Bed) <= 0)
        return false;

    // Ensure there is at least one unreserved bed.
    bool any = false;
    for (const colony::pf::IVec2& p : m_bedCells)
    {
        if (!inBounds(p.x, p.y))
            continue;
        const Cell& c = cell(p.x, p.y);
        if (c.built == TileType::Bed && c.bedReservedBy == -1)
        {
            any = true;
            break;
        }
    ...
    // Ensure scratch arrays are sized.
    if (m_nearestDist.size() != n)
        m_nearestDist.assign(n, std::numeric_limits<float>::infinity());
    if (m_nearestParent.size() != n)
        m_nearestParent.assign(n, colony::pf::kInvalidNodeId);
    if (m_nearestStamp.size() != n)
        m_nearestStamp.assign(n, 0u);

    // Bump stamp.
    ++m_nearestStampValue;
    if (m_nearestStampValue == 0u)
    {
        std::fill(m_nearestStamp.begin(), m_nearestStamp.end(), 0u);
        m_nearestStampValue = 1u;
    }
    const std::uint32_t stamp = m_nearestStampValue;

    auto getDist = [&](colony::pf::NodeId id) -> float {
        const std::size_t idx = static_cast<std::size_t>(id);
        return (m_nearestStamp[idx] == stamp) ? m_nearestDist[idx] : std::numeric_limits<float>::infinity();
    };
    auto setNode = [&](colony::pf::NodeId id, float d, colony::pf::NodeId parent) {
        const std::size_t idx = static_cast<std::size_t>(id);
        m_nearestStamp[idx] = stamp;
        m_nearestDist[idx] = d;
        m_nearestParent[idx] = parent;
    };
    ...
    const colony::pf::NodeId sid = colony::pf::to_id(startX, startY, w);
    setNode(sid, 0.0f, colony::pf::kInvalidNodeId);
    open.push({0.0f, sid});

    colony::pf::NodeId foundId = colony::pf::kInvalidNodeId;
    colony::pf::IVec2 foundPos{-1, -1};
    ...
    while (!open.empty())
    {
        ...
        const Cell& cc = cell(cpos.x, cpos.y);
        if (cc.built == TileType::Bed && cc.bedReservedBy == -1)
        {
            foundId = cur.id;
            foundPos = cpos;
            break;
        ...
        for (int dy = -1; dy <= 1; ++dy)
        {
            for (int dx = -1; dx <= 1; ++dx)
            {
                ...
                const colony::pf::NodeId nid = colony::pf::to_id(nx, ny, w);
                const float nd = cur.d + m_nav.step_cost(cpos.x, cpos.y, nx, ny);
                if (nd < getDist(nid))
                {
                    setNode(nid, nd, cur.id);
                    open.push({nd, nid});
    ...
    if (foundId == colony::pf::kInvalidNodeId)
        return false;

    outBedX = foundPos.x;
    outBedY = foundPos.y;

    // Reconstruct path from found to start, then reverse.
    std::vector<colony::pf::IVec2> rev;
    colony::pf::NodeId cur = foundId;
    while (cur != colony::pf::kInvalidNodeId)
    {
        rev.push_back(colony::pf::from_id(cur, w));
        cur = m_nearestParent[static_cast<std::size_t>(cur)];
    }
...
void World::assignEatJobs(double dtSeconds)
{
    ...
    for (Colonist& c : m_colonists)
    {
        ...
        if (c.jobKind == Colonist::JobKind::BuildPlan || c.jobKind == Colonist::JobKind::ManualMove || c.jobKind == Colonist::JobKind::HaulWood || c.jobKind == Colonist::JobKind::Sleep)
            cancelJob(c);
    }
...
void World::assignSleepJobs(double dtSeconds)
{
    if (dtSeconds <= 0.0)
        return;

    // Throttle sleep assignment similarly to other global assignment passes to avoid
    // thrashing when many colonists become tired simultaneously.
    if (m_sleepAssignCooldown > 0.0)
    {
        m_sleepAssignCooldown -= dtSeconds;
        if (m_sleepAssignCooldown > 0.0)
            return;
    }
    m_sleepAssignCooldown = kSleepAssignIntervalSeconds;
    ...
    // Hunger outranks rest: do not assign sleep to colonists that are hungry enough
    // to want to eat.
    const float eatThreshold = static_cast<float>(std::max(0.0, colonistEatThresholdPersonalFood));

    bool anyNeedSleep = false;
    for (const Colonist& c : m_colonists)
    {
        if (c.hasJob)
            continue;
        if (c.personalFood <= eatThreshold)
            continue;
        if (c.personalRest <= sleepThreshold)
        {
            anyNeedSleep = true;
            break;
        }
    ...
    const std::uint32_t bedStamp = buildBedField();
    ...
    for (Colonist& c : m_colonists)
    {
        ...
        if (bedStamp != 0 && queryBedField(bedStamp, sx, sy, bedX, bedY, path))
        {
            if (inBounds(bedX, bedY))
            {
                const Cell& bc = cell(bedX, bedY);
                if (bc.built == TileType::Bed && bc.bedReservedBy == -1)
                    found = true;
            }

            if (found)
                ++m_pathStats.sleepFieldAssigned;
            else
                path.clear();
        }
        ...
        if (!found)
        {
            if (findPathToNearestAvailableBed(sx, sy, bedX, bedY, path))
            {
                found = true;
                ++m_pathStats.sleepFieldFallback;
            }
        }

        if (!found)
        {
            // No bed available; sleep in place.
            bedX = sx;
            bedY = sy;
            path.clear();
            path.push_back({sx, sy});
        ...
        // Reserve the bed, if applicable.
        if (inBounds(bedX, bedY))
        {
            Cell& t = cell(bedX, bedY);
            if (t.built == TileType::Bed && t.bedReservedBy == -1)
                t.bedReservedBy = c.id;
        }

        c.hasJob = true;
        c.jobKind = Colonist::JobKind::Sleep;
        c.targetX = bedX;
        c.targetY = bedY;
        c.path = std::move(path);
        c.pathIndex = 0;
...
void World::stepColonist(Colonist& c, double dtSeconds)
{
    ...
    if (c.path.empty())
    {
        // Special-case: "eat/sleep in place" jobs target the tile we're already on.
        const int sx = static_cast<int>(std::floor(c.x));
        const int sy = static_cast<int>(std::floor(c.y));

        if ((c.jobKind == Colonist::JobKind::Eat || c.jobKind == Colonist::JobKind::Sleep)
            && sx == c.targetX && sy == c.targetY && inBounds(sx, sy))
        {
        ...
        // Try to recompute.
        const bool ok = (c.jobKind == Colonist::JobKind::ManualMove
                         || c.jobKind == Colonist::JobKind::HaulWood
                         || c.jobKind == Colonist::JobKind::Sleep)
...
void World::stepSleepingIfReady(Colonist& c, double dtSeconds)
{
    if (dtSeconds <= 0.0)
        return;

    if (!c.hasJob || c.jobKind != Colonist::JobKind::Sleep)
        return;
    ...
    const int sx = static_cast<int>(std::floor(c.x));
    const int sy = static_cast<int>(std::floor(c.y));
    if (sx != c.targetX || sy != c.targetY)
        return;

    const float maxRest = static_cast<float>(std::max(0.0, colonistMaxPersonalRest));
    if (maxRest <= 0.0f)
    {
        // Rest system disabled; don't get stuck sleeping forever.
        cancelJob(c);
        return;
    }

    const Cell& t = cell(c.targetX, c.targetY);
    const bool inBed = (t.built == TileType::Bed);

    const float restoreRate = static_cast<float>(std::max(0.0,
        inBed ? colonistSleepRestorePerSecond : colonistSleepRestorePerSecondNoBed));
    if (restoreRate <= 0.0f)
    {
        cancelJob(c);
        return;
    ...
    const float dtF = static_cast<float>(dtSeconds);
    c.personalRest = std::min(maxRest, c.personalRest + restoreRate * dtF);

    // Wake up when fully rested.
    if (c.personalRest >= maxRest - 1e-4f)
        cancelJob(c);
}
...
void World::cancelJob(Colonist& c) noexcept
{
    ...
    if (c.hasJob && c.jobKind == Colonist::JobKind::Sleep)
    {
        // Release any bed reservation we held.
        if (inBounds(c.targetX, c.targetY))
        {
            Cell& t = cell(c.targetX, c.targetY);
            if (t.bedReservedBy == c.id)
                t.bedReservedBy = -1;
        }
    }
...
void World::applyPlanIfComplete(int targetX, int targetY) noexcept
{
    ...
    if (oldBuilt == TileType::Bed)
        bedCacheRemove(targetX, targetY);
...
void World::rebuildBedCache() noexcept
{
    m_bedCells.clear();
    m_bedIndex.assign(static_cast<std::size_t>(m_w * m_h), -1);
...
void World::bedCacheAdd(int x, int y) noexcept
{
    if (!inBounds(x, y))
        return;
    const std::size_t flat = idx(x, y);

    if (flat >= m_bedIndex.size())
        return;

    if (m_bedIndex[flat] != -1)
        return; // already tracked

    const int newIndex = static_cast<int>(m_bedCells.size());
    m_bedCells.push_back({x, y});
    m_bedIndex[flat] = newIndex;
}

void World::bedCacheRemove(int x, int y) noexcept
{
    if (!inBounds(x, y))
        return;
    const std::size_t flat = idx(x, y);
    ...
    const int index = m_bedIndex[flat];
    if (index < 0)
        return;

    const int last = static_cast<int>(m_bedCells.size()) - 1;
    if (index != last)
    {
        const colony::pf::IVec2 moved = m_bedCells[static_cast<std::size_t>(last)];
        m_bedCells[static_cast<std::size_t>(index)] = moved;
        const std::size_t movedFlat = idx(moved.x, moved.y);
        if (movedFlat < m_bedIndex.size())
            m_bedIndex[movedFlat] = index;
    }
