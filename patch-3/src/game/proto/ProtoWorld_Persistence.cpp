...
[[nodiscard]] TileType BuiltTileFromInt(int v) noexcept
{
    ...
    const int maxBuilt = static_cast<int>(TileType::Bed);
    if (v > maxBuilt)
        v = maxBuilt;
...
[[nodiscard]] TileType PlanTileFromInt(int v) noexcept
{
    ...
    const int maxPlan = static_cast<int>(TileType::Bed);
    if (v > maxPlan)
        v = maxPlan;
...
bool World::SaveJson(const std::filesystem::path& path, std::string* outError) const noexcept
{
    try
    {
        ...
        j["tuning"] = {
            ...
            // Sleep/rest + day/night
            {"restPerColonistPerSecond", restPerColonistPerSecond},
            {"colonistMaxPersonalRest", colonistMaxPersonalRest},
            {"colonistSleepThresholdRestDay", colonistSleepThresholdRestDay},
            {"colonistSleepThresholdRestNight", colonistSleepThresholdRestNight},
            {"colonistSleepRestorePerSecond", colonistSleepRestorePerSecond},
            {"colonistSleepRestorePerSecondNoBed", colonistSleepRestorePerSecondNoBed},
            {"dayLengthSeconds", dayLengthSeconds},
            {"nightStart01", nightStart01},
            {"nightEnd01", nightEnd01},
        };
...
bool World::LoadJson(const std::filesystem::path& path, std::string* outError) noexcept
{
    try
    {
        ...
        // World time (v13+; optional).
        m_timeSeconds = ObjDouble(j, "timeSeconds", m_timeSeconds);
        if (m_timeSeconds < 0.0)
            m_timeSeconds = 0.0;
        ...
        if (j.contains("tuning") && j["tuning"].is_object())
        {
            ...
            // Sleep/rest + day/night
            restPerColonistPerSecond = ObjDouble(t, "restPerColonistPerSecond", restPerColonistPerSecond);
            colonistMaxPersonalRest = ObjDouble(t, "colonistMaxPersonalRest", colonistMaxPersonalRest);
            colonistSleepThresholdRestDay = ObjDouble(t, "colonistSleepThresholdRestDay", colonistSleepThresholdRestDay);
            colonistSleepThresholdRestNight = ObjDouble(t, "colonistSleepThresholdRestNight", colonistSleepThresholdRestNight);
            colonistSleepRestorePerSecond = ObjDouble(t, "colonistSleepRestorePerSecond", colonistSleepRestorePerSecond);
            colonistSleepRestorePerSecondNoBed = ObjDouble(t, "colonistSleepRestorePerSecondNoBed", colonistSleepRestorePerSecondNoBed);
            dayLengthSeconds = ObjDouble(t, "dayLengthSeconds", dayLengthSeconds);
            nightStart01 = ObjDouble(t, "nightStart01", nightStart01);
            nightEnd01 = ObjDouble(t, "nightEnd01", nightEnd01);
            ...
            restPerColonistPerSecond = std::max(0.0, restPerColonistPerSecond);
            colonistMaxPersonalRest = std::max(0.0, colonistMaxPersonalRest);
            colonistSleepThresholdRestDay = std::clamp(colonistSleepThresholdRestDay, 0.0, colonistMaxPersonalRest);
            colonistSleepThresholdRestNight = std::clamp(colonistSleepThresholdRestNight, 0.0, colonistMaxPersonalRest);
            colonistSleepRestorePerSecond = std::max(0.0, colonistSleepRestorePerSecond);
            colonistSleepRestorePerSecondNoBed = std::max(0.0, colonistSleepRestorePerSecondNoBed);
            dayLengthSeconds = std::max(1.0, dayLengthSeconds);
            nightStart01 = std::clamp(nightStart01, 0.0, 1.0);
            nightEnd01 = std::clamp(nightEnd01, 0.0, 1.0);
        ...
        if (j.contains("colonists") && j["colonists"].is_array())
        {
            ...
            for (const auto& item : arr)
            {
                ...
                // Rest (v13+). For legacy saves, start colonists at full.
                const float maxPersonalRest = static_cast<float>(std::max(0.0, colonistMaxPersonalRest));
                if (version >= 13)
                {
                    const float pr = static_cast<float>(ObjDouble(item, "personalRest", maxPersonalRest));
                    c.personalRest = clampf(pr, 0.0f, maxPersonalRest);
                }
                else
                {
